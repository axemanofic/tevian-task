...
db = PostgresqlDatabase("test", user="postgres", password="postgres", host="postgres")
...
def get_user():
    user_jsons = []
    for t in User.select():
        user_jsons.append(model_to_dict(t, recurse=True))
...
def create_task():
    ...
    task_image_path = os.path.join("storage", "task_images", str(task.id) + ".jpg")
...
def get_task_image(task_id):
    ...
    if task.owner.id != g.user.id:
        abort(403)
    return send_file(os.path.join("storage", "task_images", str(task.id) + ".jpg"))
...
def get_total_tasks_created():
    return jsonify({"total_tasks_created": sum(u.tasks_number for u in User.select())})
